<template>
    <div class="chart-container">
        ...
        <div v-if="moreInformation" class="container-card">
            <div class="card" v-for="o in owners">Chassis {{ o.chassis }} <br> <b style="background-color: #548644;">{{ o.owner }}</b></div>
        </div>
        </div>
</template>
  ...
  export default defineComponent({
    ...
    methods: {
          ...
          async getDataOfGraphic() {
            ...
            const response = await axios.get('http://localhost:8080/data-graphic-owners');
              
  
              this.data_owners = response.data.map((item: any) => ({
                id: item.owner_id,
                data: item.data_owner,
